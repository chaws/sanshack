...
public class ParseConfig
{
  ...
  ParseConfig(Map<String, Object> paramMap)
  {
    this.params = Collections.unmodifiableMap(paramMap);
  }
  
  static ParseConfig decode(JSONObject paramJSONObject, ParseDecoder paramParseDecoder)
  {
    paramJSONObject = (Map)((Map)paramParseDecoder.decode(paramJSONObject)).get("params");
    if (paramJSONObject == null) {
      throw new RuntimeException("Object did not contain the 'params' key.");
    }
    return new ParseConfig(paramJSONObject);
  }
  ...
  private static j<ParseConfig> getAsync(j<Void> paramj)
  {
    ParseUser.getCurrentSessionTokenAsync().d(new h()
    {
      public j<ParseConfig> then(final j<String> paramAnonymousj)
      {
        paramAnonymousj = (String)paramAnonymousj.f();
        this.val$toAwait.b(new h()
        {
          public j<ParseConfig> then(j<Void> paramAnonymous2j)
          {
            return ParseConfig.getConfigController().getAsync(paramAnonymousj);
          }
        });
      }
    });
  ...
  public static j<ParseConfig> getInBackground()
  {
    taskQueue.enqueue(new h()
    {
      public j<ParseConfig> then(j<Void> paramAnonymousj)
      {
        return ParseConfig.getAsync(paramAnonymousj);
      }
    });
  }
...
/* Location:              /home/cdo/tmp/sanshack/SantaGram_4.2-dex2jar.jar!/com/parse/ParseConfig.class
